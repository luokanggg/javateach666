package com.ctbu.javateach666.pojo.bo;

public class TeacourseBo_zxy {
	
	@Override
	public String toString() {
		return "TeacourseBo_zxy [id=" + id + ", teaname=" + teaname + ", teaid=" + teaid + ", couname=" + couname
				+ ", couyear=" + couyear + ", semester=" + semester + ", counumber=" + counumber + ", couaddress="
				+ couaddress + ", coutime=" + coutime + ", couhour=" + couhour + ", coufhour=" + coufhour + ", examid="
				+ examid + ", couid=" + couid + ", alcounumber=" + alcounumber + "]";
	}
	private int id;
	private String teaname;
	private int teaid;
	private String couname;
	private int couyear;
	private int semester;
	private int counumber;
	private String couaddress;
	private int coutime;
	private int couhour;
	private int coufhour;
	private int examid;
	private int couid;
	private int alcounumber;
	
	public int getTeaid() {
		return teaid;
	}
	public void setTeaid(int teaid) {
		this.teaid = teaid;
	}
	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public String getTeaname() {
		return teaname;
	}
	public void setTeaname(String teaname) {
		this.teaname = teaname;
	}
	public String getCouname() {
		return couname;
	}
	public void setCouname(String couname) {
		this.couname = couname;
	}
	public int getCouyear() {
		return couyear;
	}
	public void setCouyear(int couyear) {
		this.couyear = couyear;
	}
	public int getSemester() {
		return semester;
	}
	public void setSemester(int semester) {
		this.semester = semester;
	}
	public int getCounumber() {
		return counumber;
	}
	public void setCounumber(int counumber) {
		this.counumber = counumber;
	}
	public String getCouaddress() {
		return couaddress;
	}
	public void setCouaddress(String couaddress) {
		this.couaddress = couaddress;
	}
	public int getCoutime() {
		return coutime;
	}
	public void setCoutime(int coutime) {
		this.coutime = coutime;
	}
	public int getCouhour() {
		return couhour;
	}
	public void setCouhour(int couhour) {
		this.couhour = couhour;
	}
	public int getCoufhour() {
		return coufhour;
	}
	public void setCoufhour(int coufhour) {
		this.coufhour = coufhour;
	}
	public int getExamid() {
		return examid;
	}
	public void setExamid(int examid) {
		this.examid = examid;
	}
	public int getCouid() {
		return couid;
	}
	public void setCouid(int couid) {
		this.couid = couid;
	}
	public int getAlcounumber() {
		return alcounumber;
	}
	public void setAlcounumber(int alcounumber) {
		this.alcounumber = alcounumber;
	}
}
