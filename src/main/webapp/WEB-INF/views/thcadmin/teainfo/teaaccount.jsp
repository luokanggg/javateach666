...
<div id="addtea-dialog" style="width:600px;height:500px; padding:10px;">
	...
	$(function(){
	    ...
	    $("input",$("#teano").next("span")).blur(function(){
	        //alert("ok");
	    	$("#teano").textbox('setValue', $(this).val());     //给文本框赋值
	    	var teano = $("#teano").textbox('getText');     //获取文本框的值
			$.ajax({
				type: 'get',
				url: 'checkTeano?teano='+teano,
				contentType: 'application/json',
				success: function(result){
					if(result == "OK"){
						$.messager.alert('信息提示', '该教师编号已存在！请重新输入');
					}else{
